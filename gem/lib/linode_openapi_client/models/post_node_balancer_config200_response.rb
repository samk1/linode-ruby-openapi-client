...
module LinodeOpenapiClient
  ...
  class PostNodeBalancerConfig200Response
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinodeOpenapiClient::PostNodeBalancerConfig200Response` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinodeOpenapiClient::PostNodeBalancerConfig200Response`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [ssl_commonname, check, proxy_protocol, check_interval, check_attempts, port, ssl_fingerprint, nodes_status, ssl_cert, nodebalancer_id, stickiness, cipher_suite, check_body, protocol, algorithm, check_timeout, id, check_path, check_passive, ssl_key].hash
    end
