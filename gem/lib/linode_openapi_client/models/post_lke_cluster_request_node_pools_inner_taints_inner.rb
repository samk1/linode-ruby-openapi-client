...
module LinodeOpenapiClient
  class PostLkeClusterRequestNodePoolsInnerTaintsInner
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinodeOpenapiClient::PostLkeClusterRequestNodePoolsInnerTaintsInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinodeOpenapiClient::PostLkeClusterRequestNodePoolsInnerTaintsInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @value.to_s.length > 63
      return false if @value.to_s.length < 0
      return false if @value !~ Regexp.new(/^(([A-Za-z0-9][-A-Za-z0-9_.]*)?[A-Za-z0-9])?/)
      return false if @key.nil?
      return false if @key.to_s.length > 63
      return false if @key.to_s.length < 1
      return false if @key !~ Regexp.new(/^([A-Za-z0-9][-A-Za-z0-9_.]*)?(\/)?([A-Za-z0-9][-A-Za-z0-9_.]{1,63})?$/)
