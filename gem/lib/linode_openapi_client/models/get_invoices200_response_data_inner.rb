...
module LinodeOpenapiClient
  ...
  class GetInvoices200ResponseDataInner
    ...
    def self.openapi_types
      {
        ...
        :'tax' => :'Float',
        :'tax_summary' => :'Array<GetInvoices200ResponseDataInnerTaxSummaryInner>',
        :'label' => :'String',
        :'subtotal' => :'Float'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinodeOpenapiClient::GetInvoices200ResponseDataInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinodeOpenapiClient::GetInvoices200ResponseDataInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'tax_summary')
        if (value = attributes[:'tax_summary']).is_a?(Array)
          self.tax_summary = value
        end
      end
    ...
    def hash
      [total, id, date, billing_source, tax, tax_summary, label, subtotal].hash
    end
