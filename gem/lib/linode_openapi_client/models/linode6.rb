...
module LinodeOpenapiClient
  class Linode6
    ...
    def self.openapi_types
      {
        :'image' => :'String',
        :'placement_group' => :'Linode6PlacementGroup',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinodeOpenapiClient::Linode6` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinodeOpenapiClient::Linode6`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [image, placement_group, has_user_data, watchdog_enabled, type, label, updated, ipv6, specs, id, tags, hypervisor, created, ipv4, region, backups, host_uuid, alerts, group, status].hash
    end
