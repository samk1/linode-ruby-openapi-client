...
module LinodeOpenapiClient
  ...
  class GetLkeClusterPools200ResponseDataInnerLabels
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinodeOpenapiClient::GetLkeClusterPools200ResponseDataInnerLabels` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinodeOpenapiClient::GetLkeClusterPools200ResponseDataInnerLabels`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      pattern = Regexp.new(/^([A-Za-z0-9][-A-Za-z0-9_.]*){0,62}?(\/)?([A-Za-z0-9][-A-Za-z0-9_.]{0,62})?$/)
    ...
    def valid?
      ...
      return false if !@key.nil? && @key !~ Regexp.new(/^([A-Za-z0-9][-A-Za-z0-9_.]*){0,62}?(\/)?([A-Za-z0-9][-A-Za-z0-9_.]{0,62})?$/)
      true
    end
