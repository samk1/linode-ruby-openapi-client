...
module LinodeOpenapiClient
  ...
  class PutFirewall200ResponseRules
    ...
    def self.openapi_types
      {
        ...
        :'inbound' => :'Array<PutFirewall200ResponseRulesInboundInner>',
        :'outbound' => :'Array<PutFirewall200ResponseRulesOutboundInner>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinodeOpenapiClient::PutFirewall200ResponseRules` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinodeOpenapiClient::PutFirewall200ResponseRules`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [outbound_policy, inbound_policy, inbound, outbound].hash
    end
