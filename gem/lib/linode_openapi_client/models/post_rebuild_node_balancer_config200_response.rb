...
module LinodeOpenapiClient
  ...
  class PostRebuildNodeBalancerConfig200Response
    ...
    def self.openapi_types
      {
        ...
        :'nodes_status' => :'PostNodeBalancerConfigRequestNodesStatus',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinodeOpenapiClient::PostRebuildNodeBalancerConfig200Response` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinodeOpenapiClient::PostRebuildNodeBalancerConfig200Response`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [check_body, cipher_suite, stickiness, protocol, check_timeout, algorithm, check_passive, ssl_key, check_path, id, check, ssl_commonname, check_interval, proxy_protocol, nodes_status, ssl_fingerprint, check_attempts, port, nodebalancer_id, ssl_cert].hash
    end
