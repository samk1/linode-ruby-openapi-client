...
module LinodeOpenapiClient
  ...
  class PutStackScript200Response
    ...
    def self.openapi_types
      {
        :'rev_note' => :'String',
        :'user_defined_fields' => :'Array<PutStackScript200ResponseUserDefinedFieldsInner>',
        :'username' => :'String',
        :'is_public' => :'Boolean',
        :'created' => :'Time',
        :'mine' => :'Boolean',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinodeOpenapiClient::PutStackScript200Response` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinodeOpenapiClient::PutStackScript200Response`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [rev_note, user_defined_fields, username, is_public, created, mine, id, updated, description, label, deployments_active, deployments_total, user_gravatar_id, script, images].hash
    end
