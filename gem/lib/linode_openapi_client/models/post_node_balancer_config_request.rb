...
module LinodeOpenapiClient
  ...
  class PostNodeBalancerConfigRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinodeOpenapiClient::PostNodeBalancerConfigRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinodeOpenapiClient::PostNodeBalancerConfigRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [check_timeout, algorithm, check_passive, ssl_key, id, check_path, cipher_suite, check_body, stickiness, protocol, nodes_status, check_attempts, port, ssl_fingerprint, nodebalancer_id, ssl_cert, check, ssl_commonname, check_interval, proxy_protocol].hash
    end
