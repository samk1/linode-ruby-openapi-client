...
module LinodeOpenapiClient
  ...
  class GetLinodeFirewalls200ResponseDataInnerRules
    ...
    def self.openapi_types
      {
        :'outbound' => :'Array<GetLinodeFirewalls200ResponseDataInnerRulesOutboundInner>',
        :'inbound' => :'Array<GetLinodeFirewalls200ResponseDataInnerRulesInboundInner>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinodeOpenapiClient::GetLinodeFirewalls200ResponseDataInnerRules` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinodeOpenapiClient::GetLinodeFirewalls200ResponseDataInnerRules`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] inbound_policy Object to be assigned
    def inbound_policy=(inbound_policy)
      validator = EnumAttributeValidator.new('String', ["ACCEPT", "DROP"])
      unless validator.valid?(inbound_policy)
        fail ArgumentError, "invalid value for \"inbound_policy\", must be one of #{validator.allowable_values}."
      end
      @inbound_policy = inbound_policy
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] outbound_policy Object to be assigned
    def outbound_policy=(outbound_policy)
      validator = EnumAttributeValidator.new('String', ["ACCEPT", "DROP"])
      unless validator.valid?(outbound_policy)
        fail ArgumentError, "invalid value for \"outbound_policy\", must be one of #{validator.allowable_values}."
      end
      @outbound_policy = outbound_policy
    end
    ...
    def hash
      [outbound, inbound, inbound_policy, outbound_policy].hash
    end
