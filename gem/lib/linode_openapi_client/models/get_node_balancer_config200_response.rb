...
module LinodeOpenapiClient
  ...
  class GetNodeBalancerConfig200Response
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinodeOpenapiClient::GetNodeBalancerConfig200Response` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinodeOpenapiClient::GetNodeBalancerConfig200Response`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [nodebalancer_id, ssl_cert, nodes_status, ssl_fingerprint, port, check_attempts, check_interval, proxy_protocol, check, ssl_commonname, check_passive, ssl_key, check_path, id, check_timeout, algorithm, protocol, check_body, cipher_suite, stickiness].hash
    end
