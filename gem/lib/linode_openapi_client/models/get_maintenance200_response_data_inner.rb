...
module LinodeOpenapiClient
  ...
  class GetMaintenance200ResponseDataInner
    ...
    def self.openapi_types
      {
        :'entity' => :'GetMaintenance200ResponseDataInnerEntity',
        :'type' => :'String',
        :'_when' => :'Time',
        :'reason' => :'String',
        :'status' => :'String'
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinodeOpenapiClient::GetMaintenance200ResponseDataInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinodeOpenapiClient::GetMaintenance200ResponseDataInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["reboot", "cold_migration", "live_migration"])
      return false unless type_validator.valid?(@type)
      status_validator = EnumAttributeValidator.new('String', ["completed", "pending", "started"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["reboot", "cold_migration", "live_migration"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["completed", "pending", "started"])
    ...
    def hash
      [entity, type, _when, reason, status].hash
    end
