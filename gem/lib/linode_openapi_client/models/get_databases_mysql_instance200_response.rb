...
module LinodeOpenapiClient
  ...
  class GetDatabasesMysqlInstance200Response
    ...
    def self.openapi_types
      {
        :'port' => :'Integer',
        :'engine' => :'String',
        :'updates' => :'GetDatabasesMysqlInstance200ResponseUpdates',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinodeOpenapiClient::GetDatabasesMysqlInstance200Response` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinodeOpenapiClient::GetDatabasesMysqlInstance200Response`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [port, engine, updates, ssl_connection, replication_type, version, updated, type, label, encrypted, created, region, id, used_disk_size_gb, total_disk_size_gb, cluster_size, status, hosts, allow_list].hash
    end
