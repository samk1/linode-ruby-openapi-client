...
module LinodeOpenapiClient
  ...
  class GetManagedLinodeSettings200ResponseDataInnerSsh
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinodeOpenapiClient::GetManagedLinodeSettings200ResponseDataInnerSsh` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinodeOpenapiClient::GetManagedLinodeSettings200ResponseDataInnerSsh`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@user.nil? && @user.to_s.length > 32
        invalid_properties.push('invalid value for "user", the character length must be smaller than or equal to 32.')
      end

      if !@user.nil? && @user.to_s.length < 0
        invalid_properties.push('invalid value for "user", the character length must be great than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if !@user.nil? && @user.to_s.length > 32
      return false if !@user.nil? && @user.to_s.length < 0
    ...
    def user=(user)
      if !user.nil? && user.to_s.length > 32
        fail ArgumentError, 'invalid value for "user", the character length must be smaller than or equal to 32.'
      end

      if !user.nil? && user.to_s.length < 0
        fail ArgumentError, 'invalid value for "user", the character length must be great than or equal to 0.'
      end
