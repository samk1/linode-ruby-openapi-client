...
module LinodeOpenapiClient
  ...
  class AddedGetEvents200DataInner
    ...
    def self.openapi_types
      {
        :'entity' => :'AddedGetEvents200DataInnerEntity',
        :'action' => :'String',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinodeOpenapiClient::AddedGetEvents200DataInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinodeOpenapiClient::AddedGetEvents200DataInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [entity, action, time_remaining, rate, percent_complete, seen, username, secondary_entity, duration, status, id, read, created, message].hash
    end
