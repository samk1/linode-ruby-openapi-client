...
module LinodeOpenapiClient
  ...
  class AddedGetEntityTransfers200AllOfData
    ...
    def self.openapi_types
      {
        ...
        :'token' => :'String',
        :'created' => :'Time',
        :'entities' => :'AddedGetEntityTransfers200AllOfEntities',
        :'is_sender' => :'Boolean',
        :'expiry' => :'Time'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinodeOpenapiClient::AddedGetEntityTransfers200AllOfData` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinodeOpenapiClient::AddedGetEntityTransfers200AllOfData`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [updated, status, token, created, entities, is_sender, expiry].hash
    end
