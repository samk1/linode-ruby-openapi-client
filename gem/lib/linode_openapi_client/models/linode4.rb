  ...
  class Linode4
    ...
    def self.openapi_types
      {
        ...
        :'placement_group' => :'Linode4PlacementGroup',
        :'image' => :'String',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinodeOpenapiClient::Linode4` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinodeOpenapiClient::Linode4`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [alerts, host_uuid, backups, status, group, hypervisor, tags, id, specs, ipv6, region, created, ipv4, type, label, updated, has_user_data, placement_group, image, watchdog_enabled].hash
    end
