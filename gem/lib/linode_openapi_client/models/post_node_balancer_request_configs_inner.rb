...
module LinodeOpenapiClient
  ...
  class PostNodeBalancerRequestConfigsInner
    ...
    def self.openapi_types
      {
        ...
        :'nodes' => :'Array<PostNodeBalancerRequestConfigsInnerNodesInner>',
        :'algorithm' => :'String',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinodeOpenapiClient::PostNodeBalancerRequestConfigsInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinodeOpenapiClient::PostNodeBalancerRequestConfigsInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [check_path, ssl_key, check_passive, nodes, algorithm, check_timeout, protocol, stickiness, cipher_suite, check_body, ssl_cert, check_attempts, port, proxy_protocol, check_interval, check].hash
    end
