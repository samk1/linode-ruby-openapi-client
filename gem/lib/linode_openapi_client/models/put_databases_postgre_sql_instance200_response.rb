...
module LinodeOpenapiClient
  ...
  class PutDatabasesPostgreSqlInstance200Response
    ...
    def self.openapi_types
      {
        ...
        :'engine' => :'String',
        :'updates' => :'PutDatabasesPostgreSqlInstance200ResponseUpdates',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinodeOpenapiClient::PutDatabasesPostgreSqlInstance200Response` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinodeOpenapiClient::PutDatabasesPostgreSqlInstance200Response`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [port, replication_commit_type, engine, updates, ssl_connection, replication_type, updated, version, label, type, encrypted, created, region, id, used_disk_size_gb, total_disk_size_gb, cluster_size, hosts, status, allow_list].hash
    end
