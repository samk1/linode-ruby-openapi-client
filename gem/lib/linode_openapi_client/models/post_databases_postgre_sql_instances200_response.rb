...
module LinodeOpenapiClient
  ...
  class PostDatabasesPostgreSqlInstances200Response
    ...
    def self.openapi_types
      {
        ...
        :'port' => :'Integer',
        :'updates' => :'PostDatabasesPostgreSqlInstances200ResponseUpdates',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `LinodeOpenapiClient::PostDatabasesPostgreSqlInstances200Response` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `LinodeOpenapiClient::PostDatabasesPostgreSqlInstances200Response`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [type, label, encrypted, replication_type, version, updated, ssl_connection, port, updates, replication_commit_type, engine, allow_list, cluster_size, total_disk_size_gb, hosts, status, id, used_disk_size_gb, created, region].hash
    end
